...
export default function Home() {
  ...
  const runAgents = () => {
    ...
    const url = `/api/run-task/stream?task=${encodeURIComponent(task)}`
    ...
    es.onerror = () => {
      setError('Connection to backend lost. Make sure the FastAPI server is running on port 8000.')
      setPhase('error')
      es.close()
    }
  }
            ...
            <div className="space-y-3">
              {steps.map((step) => (
                <div
                  ...
                  className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-3 animate-fade-in"
                >
